...
def compute_false_negatives(target,predictions):  
	...
	return df[(df["target"]==1) & (df["predictions"]==0)].shape[0]*1.0/(df[(df["target"]==1)].shape[0]+1)


def compute_false_positives(target,predictions):  
	df = pd.DataFrame({"target":target,"predictions":predictions})
	return df[(df["target"]==0) & (df["predictions"]==1)].shape[0]*1.0/(df[(df["target"]==0)].shape[0]+1)
